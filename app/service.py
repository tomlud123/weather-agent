...
async def _fetch_current_weather_with_client(city: str, client) -> Optional[Weather]:
    """Fetch current weather using provided client (for concurrent requests)."""
    api_key = get_api_key()
    if not api_key:
        return None
    ...
    try:
        ...
        print(f"[error] Network issue for {city}: {exc}", file=sys.stderr)
        return None
    ...
    try:
        ...
        print(f"[error] Unexpected response structure for {city}: {exc}", file=sys.stderr)
        return None


async def _fetch_city_bundle_with_client(city: str, days: int, client) -> Optional[CityWeatherBundle]:
    """Fetch city bundle using provided client (for concurrent requests)."""
    api_key = get_api_key()
    if not api_key:
        return None
    ...
    try:
        ...
        print(f"[error] Network error for {city}: {exc}", file=sys.stderr)
        return None
    ...
    # Build DailyForecast for the next `days` dates
    daily_forecasts: List[DailyForecast] = []
    for date_iso in sorted(by_date.keys())[: max(0, days)]:
        bucket = by_date[date_iso]
        temp_vals: List[float] = []
...
async def fetch_many_cities(cities: Iterable[str]) -> List[Optional[Weather]]:
    ...
    async with await get_client() as client:
        tasks = [_fetch_current_weather_with_client(city, client) for city in cities]
...
async def fetch_many_bundles(cities: Iterable[str], days: int = 2) -> List[Optional[CityWeatherBundle]]:
    ...
    async with await get_client() as client:
        tasks = [_fetch_city_bundle_with_client(city, days, client) for city in cities]
        results = await asyncio.gather(*tasks, return_exceptions=False)
        return list(results)
